...
class SearchGuestByFullnameForm(forms.ModelForm): 
    ...
    def clean_fullname(self):
        fullname = self.cleaned_data['fullname']
        return fullname.strip()  # Loại bỏ các khoảng trắng ở đầu và cuối chuỗi

# class SearchGuestByDateForm(forms.ModelForm):
#     class Meta:
#         model = Guests
#         fields = ['date']
        
class SearchGuestByMonthYearForm(forms.Form): # dùng đc type number
    month = forms.IntegerField(label='Tháng')
    year = forms.IntegerField(label='Năm') 
    
# class SearchGuestByMonthYearForm(forms.Form):
#     month_year = forms.DateField(label='Tháng và Năm', widget=forms.DateInput(attrs={'type': 'month'}))
